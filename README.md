...
If you deployed to HF Spaces (https://huggingface.co/spaces/YOUR-USERNAME/Cerberus), configure these environment variables in your Space settings instead of the `.env` file:
...
Then configure your VS Code extension:
Go to **Settings > Cerberus: Backend Url** and set it to `https://YOUR-USERNAME-cerberus.hf.space`
...
- Verify that your `n8n` workflow is Active and the Webhook URL in your `.env` matches.
